...
def spring_model(AC, T_A, NL_A, T_C, NL_C, m, k, offset, A=0, omega=0, g=9.8, dp=2, time=[0,10],plot=True, animate=False, energy=False):
    ...
    a = round (k / m, 2)
    b = round( (T_A + T_C) / m , 2)
    c = round(-m*g + T_A * e_A - T_C * e_C, 0)

    def system(t, state, a, b):
        X, v = state

        dX_dt = v
        dv_dt = -a*v - b*X + c + A*np.sin(omega*t)
    ...
    def E_lost(x):
        ...
        return np.cumsum(k*v_values**2)*dt
...
#Original problem
"spring_modelling(AC=8, T_A=8, NL_A=2, T_C=3, NL_C=3, m=0.5, k=2, offset=0.6)"
